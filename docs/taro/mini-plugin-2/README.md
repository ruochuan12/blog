...
graph TD;
    A[开始] --> B[获取文件路径和配置路径];
    B --> C[读取配置文件];
    C --> D{是否为构建插件且有 componentGenerics};
    D -- 是 --> E[处理 componentGenerics];
    D -- 否 --> F{是否有 usingComponents};
    E --> F;
    F -- 是 --> G[遍历 usingComponents];
    F -- 否 --> K[保存配置到 filesConfig];
    G --> H{是否为别名路径};
    H -- 是 --> I[替换别名路径];
    H -- 否 --> J{是否为第三方依赖};
    I --> J;
    ...
    J2 --> J3;
    J3 --> J4[添加到依赖组件列表];
    J4 --> J5{是否为第三方组件且非原生文件};
    J5 -- 是 --> J6[添加到第三方组件集合];
    J5 -- 否 --> J7[跳过];
    J6 --> J8{是否遍历完 usingComponents};
    J7 --> J8;
    J8 -- 否 --> G;
    J8 -- 是 --> J9[遍历依赖组件列表];
    J9 --> J10{文件是否存在且未添加到 components};
    J10 -- 是 --> J11{是否为本地化组件};
    J10 -- 否 --> J12[跳过];
    J11 -- 是 --> J12;
    J11 -- 否 --> J13[获取组件信息];
    J13 --> J14[添加到独立分包组件列表];
    J14 --> J15[添加到 components 集合];
    J15 --> J16[递归调用 compileFile];
    J16 --> J17{是否遍历完依赖组件列表};
    J17 -- 否 --> J9;
    J17 -- 是 --> K;
    K --> L[结束];
